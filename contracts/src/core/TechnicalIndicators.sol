...
contract TechnicalIndicators is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            for (uint256 j = 0; j < historicalPrices[i].length; j++) {
                uint256 timestamp = startTimestamps[i] + (j * 1 days);
    ...
    function getPriceHistory(
        ...
        for (uint256 i = 0; i < prices.length; i++) {
            if (prices[i].timestamp >= startTime && startIdx == 0) {
                startIdx = i;
            }
        ...
        require(startIdx < endIdx, "Invalid time range");
